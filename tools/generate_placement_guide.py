...
ITEMS = {
    "en": (
        (1, "red_led", "RED 10 mm LED", "Ø10.2 · insert from inside", "red"),
        (2, "red_button", "RED DC184", "Ø12.0 · nut inside", "red"),
        (3, "yellow_led", "YELLOW 10 mm LED", "Ø10.2 · insert from inside", "yellow"),
        (4, "yellow_button", "YELLOW DC184", "Ø12.0 · nut inside", "yellow"),
        (5, "green_led", "GREEN 10 mm LED", "Ø10.2 · insert from inside", "green"),
        (6, "green_button", "BLACK DC180", "Ø16.0 · nut inside", "black"),
        (7, "switch_led", "BLUE 10 mm LED", "Ø10.2 · insert from inside", "blue"),
        (8, "rocker", "DC131A", "Ø20.2 · nut inside", "red"),
        (9, "dimmer_led", "WHITE 10 mm LED", "Ø10.2 · insert from inside", "white"),
        (10, "dial", "1K POT + DIAL", "bushing Ø7.0 · shaft Ø6.2", "orange"),
        (11, "buzzer", "12 mm ACTIVE BUZZER", "place in the inner cup", "purple"),
        (12, "buzzer_button", "BLUE DC180", "Ø16.0 · nut inside", "blue"),
    ),
    "tr": (
...
COPY = {
    "en": {
        "title": "ACTIVITY BOX — COMPONENT PLACEMENT",
        "subtitle": "Use this front-view layout; follow the notes below for installation direction.",
        "front": "FRONT VIEW",
        "legend": "NUMBER → COMPONENT",
        "right": "RIGHT SIDE",
        "clip": "19×13 mm · snap clips",
        "back": "BACK PLATE",
        "battery": "2×AA BATTERY HOLDER ×2 · separate rails",
        "orientation": "ASSEMBLY ORIENTATION",
        "notes": (
            "• LED lens out, flange in",
            "• Keep nuts inside the box",
            "• Pot body in, dial outside",
            "• Point buzzer toward holes",
            "• Keep wires clear of the lid",
        ),
    },
    "tr": {
        "title": "MİNİK KEŞİF KUTUSU — BİLEŞEN YERLEŞİMİ",
        "subtitle": "Yerleşim için bu önden görünüşü, montaj yönü için aşağıdaki notları kullan.",
        "front": "ÖNDEN GÖRÜNÜŞ",
        "legend": "NUMARA → TAKILACAK PARÇA",
        "right": "SAĞ YAN",
        "clip": "19×13 mm · tırnakla",
        "back": "ARKA KAPAK",
        "battery": "2×AA PİL YUVASI ×2 · ayrı raylara",
        "orientation": "MONTAJ YÖNÜ",
        "notes": (
...
def generate_svg(locale="en"):
    ...
    parts = [
        ...
        f"<style>{font_css}text{{font-family:'DejaVu Sans',sans-serif;fill:#263548}}.title{{font-size:7px;font-weight:bold}}.subtitle{{font-size:3.2px}}.view{{font-size:3.6px;font-weight:bold}}.badge{{font-size:3.2px;font-weight:bold;dominant-baseline:middle}}.legend-title{{font-size:3.1px;font-weight:bold}}.legend-detail{{font-size:2.35px}}.small{{font-size:2.7px}}.note{{font-size:2.45px}}</style>",
        f'<rect width="320" height="230" fill="{COLORS["background"]}"/>',
        svg_text(160, 10, copy["title"], "title", "middle"),
        svg_text(160, 17, copy["subtitle"], "subtitle", "middle", COLORS["muted"]),
        svg_text(FRONT_ORIGIN[0], 25, copy["front"], "view"),
    ...
    parts.extend(
        (
            ...
            svg_text(legend_x + 5, legend_y + 7, copy["legend"], "view"),
        )
    )
    ...
    parts.extend(
        (
            ...
            svg_text(24, 186, copy["right"], "view"),
            ...
            svg_text(166.5, 215, copy["battery"], "small", "middle"),
            '<rect x="234" y="179" width="78" height="39" rx="6" fill="#FFFFFF" stroke="#B9C7D3" stroke-width="0.8"/>',
            svg_text(240, 186, copy["orientation"], "view"),
            *(svg_text(240, 192 + index * 5.5, note, "note") for index, note in enumerate(copy["notes"])),
            "</svg>",
        )
    )
    return "\n".join(parts) + "\n"
...
def draw_png(locale="en"):
    ...
    panel_w = 200 * FRONT_SCALE
    panel_h = 160 * FRONT_SCALE
    draw_text(draw, (160, 10), copy["title"], 7, bold=True, anchor="mm")
    draw_text(
        draw,
        (160, 17),
        copy["subtitle"],
    ...
    draw_text(draw, (FRONT_ORIGIN[0], 25), copy["front"], 3.6, bold=True)
    ...
    draw_text(draw, (legend_x + 5, legend_y + 7), copy["legend"], 3.6, bold=True)
    for index, (number, _, label, detail, color_key) in enumerate(items):
        row_y = legend_y + 14 + index * 10.8
        draw.ellipse(
    ...
    draw_text(draw, (24, 186), copy["right"], 3.6, bold=True)
    ...
    draw_text(draw, (57.5, 215), copy["clip"], 2.7, anchor="mm")

    draw_text(draw, (112, 186), copy["back"], 3.6, bold=True)
    for x in (113, 170):
    ...
    draw_text(draw, (166.5, 215), copy["battery"], 2.7, anchor="mm")

    draw_text(draw, (240, 186), copy["orientation"], 3.6, bold=True)
    for index, note in enumerate(copy["notes"]):
...
def main():
    for locale, output in (("en", Path("docs")), ("tr", Path("docs/tr"))):
        ...
        draw_png(locale).save(
            output / "component-placement-guide.png", dpi=(DPI, DPI)
        )
    print("wrote English and Turkish component placement SVG and PNG guides")
